...
**Nexalink Tunnel** 是由 [深圳市然壹科技有限公司](https://www.run1tech.com) 开发并开源的、基于现代网络协议的去中心化安全网络隧道与穿透引擎。
...
*   **原生 QUIC 传输：** 基于 UDP 协议的多路复用（Multiplexing）设计，彻底摆脱传统的 TCP 队头阻塞。在高延迟、高丢包的跨境或弱网环境下依然能提供平滑且低延迟的传输能力。
...
Nexalink 采用 **“TCP 反向代理 + UDP 直连/打洞”** 的混合部署拓扑：

```text
       [ 外网客户端 / Client ]
             /        \
    (TCP 443 / WSS)  (UDP 7830 / QAD地址发现)
           /            \
[ Nginx / 反向代理 ]    [ 物理网络 / UDP 直接打洞 ]
          |                 |
 [ Nexalink 容器 / 443 ]    |
          \                 /
        [ 内网服务端 / Server ]
...
*   **信令与握手（TCP 443）**：外界通过标准的 443 端口连接，经 Nginx 统一进行 TLS 证书卸载与反向代理，安全、干净地转发到 Nexalink 的工作端口。
*   **打洞与穿透（UDP 7830）**：利用中继服务器的 UDP QAD 端口协助双方进行地址探测，一旦打洞成功，数据将直接通过 UDP (QUIC) 在两端之间建立直连加密通道，不再消耗中继服务器带宽。
...
所有的核心配置均已重构为**动态参数解析驱动**，生产环境中无需重复编译即可轻松通过命令行调整运行配置。
